...
`tools/list` at `https://gateway.pipeworx.io/mygene-info/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/mygene_info_query \
  -H 'Content-Type: application/json' \
  -d '{"query":"BRCA1"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/mygene_info_query`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
